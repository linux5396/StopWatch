/**
 * using std chrono steady_clock to impl
 */
#include "StopWatch.h"
...
uint64_t StopWatch::ElapsedMinutes() const {
    return std::chrono::duration_cast<minutes>(clock::now() - mStart).count();
}

uint64_t StopWatch::ElapsedHours() const {
    return std::chrono::duration_cast<hours>(clock::now() - mStart).count();
}
